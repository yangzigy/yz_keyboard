...
#### 基本概念  
usb部分分3层：
1. 硬件设备访问（f1_usb.c）负责寄存器访问抽象，依赖usb逻辑层
2. usb逻辑层（usb_core.c）负责usb描述符等管理逻辑实现，不依赖其他层，具有通用性
3. 应用层（usb_kb_m.c）负责具体到本键盘应用的usb描述符实现，usb通信收发，依赖上述两层
usb通信的基本流程是host主动，从机纯被动，所以发送都是事先准备好数据到缓冲区，等host来取。每次传输完成后端点会自动设置为NAK状态，并给中断，让应用程序准备好下次传输。
每个端点有两个分组缓冲，一个收一个发。可位于缓冲区的任意位置，由缓冲区表描述收发缓存的地址和长度，缓冲区表也在缓冲区中。

所以usb的512字节缓冲区如何用是需要应用来设计的，一般从0开始是缓冲区表，依次描述各端点的缓冲区地址和长度。然后为每个缓存分配空间。

usb逻辑层实现描述符等初始化逻辑，由于端点0的通信需要发送的数据长度大于一包长度，所以在usb_core中做了状态机分包发送

#### 发送流程  
通过keyboard_send和mouse_send函数发送数据，只是将数据放在发送缓冲区中，等host来取。并没有实现批量数据的发送功能，如果需要实现任意数据的收发，需要在此做队列缓存
